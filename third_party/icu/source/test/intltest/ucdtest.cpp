...
TESTCASE_AUTO(TestScriptExtensionsCodePoints);    TESTCASE_AUTO_END;
}
...
assertEquals("encoded, has characters", static_cast<UBool>(sc == charScript), static_cast<UBool>(!scriptSet.isEmpty()));            if(uscript_isRightToLeft(sc)) {
...
assertEquals("ID_Status(Katakana ko)=Allowed", U_ID_STATUS_ALLOWED, getIDStatus(0x30B3));
    assertEquals("ID_Status(U+29D98)=Allowed", U_ID_STATUS_ALLOWED, getIDStatus(0x29D98));    assertEquals("ID_Status(U+10FFFF)=Restricted", U_ID_STATUS_RESTRICTED, getIDStatus(0x10FFFF));
...
// Unicode 17: Only 33791 Allowed characters after making
    ...
    assertTrue("Allowed number of characters", idStatus.size() >= 33791);    assertFalse("Allowed.contains(slash)", idStatus.contains(0x2F));
...
assertTrue("Allowed.contains(Katakana small ko)", idStatus.contains(0x1B155));
    assertTrue("Allowed.contains(U+2EE5D)", idStatus.contains(0x2EE5D));    assertFalse("Allowed.contains(U+10FFFF)", idStatus.contains(0x10FFFF));
}
...
assertEquals("ID_Type(Katakana small ko)", u"Recommended", getIDTypes(0x1B155));
    assertEquals("ID_Type(U+2EE5D)", u"Recommended", getIDTypes(0x2EE5D));    assertEquals("ID_Type(U+10FFFF)", u"Not_Character", getIDTypes(0x10FFFF));
...
assertEquals("ID_Type(LATIN SMALL LETTER EZH)=", u"Technical Uncommon_Use", getIDTypes(0x0292));    assertEquals("ID_Type(MUSICAL SYMBOL KIEVAN C CLEF)", u"Not_XID Technical Uncommon_Use", getIDTypes(0x1D1DE));
...
// Unicode 17: Only 33773 Recommended characters after making
    // most Unified_Ideograph characters Uncommon_Use and revisiting others as well.
    assertTrue("Recommended number of characters", rec.size() >= 33773);    assertTrue("Inclusion number of characters", incl.size() >= 17);
...
assertTrue("Recommended.contains(U+29D98)", rec.contains(0x29D98));    assertTrue("Limited_Use.contains(SYRIAC FEMININE DOT)", limited.contains(0x0740));
