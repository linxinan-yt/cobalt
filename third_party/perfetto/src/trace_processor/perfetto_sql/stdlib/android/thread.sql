...
count(thread.start_ts) OVER (PARTITION BY upid, android_standardize_thread_name(thread.name) ORDER BY thread.start_ts RANGE BETWEEN CURRENT ROW AND cast_int!($sliding_window_dur) FOLLOWING) AS count    FROM thread
...
SELECT
  ...
  upid,
  thread_name_prefix,
  max(count) AS max_count_per_secFROM x
GROUP BY
  upid,
  thread_name_prefix
HAVING
...
RETURNS TABLE (  -- Process name creating threads.
  process_name STRING,
  -- Unique process pid creating threads.
  upid JOINID(process.id),
  -- Max number of threads created within a time window.
