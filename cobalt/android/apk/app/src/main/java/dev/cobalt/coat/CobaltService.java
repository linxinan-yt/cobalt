...
public abstract class CobaltService {
    ...
    public interface Natives {
        // Can not set it as nativeService, JNI zero has template code to convert it to a Service
        // object
    ...
    @CalledByNative
    public abstract @JniType("ResponseToClientInfo") ResponseToClient receiveFromClient(
            byte[] data);
    ...
    protected void sendToClient(long nativeService, byte[] data) {
        synchronized (lock) {
            if (!opened) {
                Log.w(
                        TAG,
                        "Platform service did not send data to client, because client already"
                                + " closed the platform service.");
                return;
            }
