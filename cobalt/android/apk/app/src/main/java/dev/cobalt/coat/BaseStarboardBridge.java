...
public class BaseStarboardBridge {
    ...
    protected void onActivityStart(Activity activity) {
        ...
        if (activeActivityCount > 0) {
            Log.w(
                    TAG,
                    String.format(
                            "onActivityStart: New activity %s starting while there are previous %d"
        ...
        if (isActivityLifecycleCoordinationEnabled()) {
            // Only resume services and trigger foreground logic on 0 -> 1 started activity
            // transition.
            if (activeActivityCount == 0) {
                beforeStartOrResume();
            }
    ...
    protected void onActivityStop(Activity activity) {
        ...
        mActivities.put(activity, Boolean.FALSE);
        if (isActivityLifecycleCoordinationEnabled()) {
            // Only suspend services and disable foreground config updates if no other activity
            // instance
            ...
            if (!hasStartedActivities()) {
                ...
                Log.i(
                        TAG,
                        "onActivityStop: Another activity is still started; skipping suspend and"
                                + " foreground change.");
            }
        } else {
            beforeSuspend();
    ...
    protected void onActivityDestroy(Activity activity) {
        ...
        if (isActivityLifecycleCoordinationEnabled()) {
            ...
            if (hasLiveActivities()) {
                Log.i(
                        TAG,
                        "Activity destroyed but another activity instance is still live; skipping"
                                + " exit check.");
                return;
            }
            ...
            // Safety net: if all activities are destroyed but services remain in mCobaltServices
            // (e.g.
            ...
            if (!mCobaltServices.isEmpty()) {
                Log.w(
                        TAG,
                        String.format(
                                "onActivityDestroy: %d CobaltService instance(s) still open after"
                                        + " last activity destroyed; force cleaning up.",
            ...
            // instance's onCreate()/onStart() to run before the old instance's onDestroy()
            // executes.
            ...
            if (mActivityHolder.get() != activity && mActivityHolder.get() != null) {
                Log.i(
                        TAG,
                        "Activity destroyed but another activity is active; skipping service"
                                + " cleanup.");
                return;
            }
            if (mActivityHolder.get() == activity) {
    ...
    /**
     * Returns the canonical path to the directory where application specific files should be
     * written, resolving symbolic links. nplb tests compare against resolved paths.
     */
     ...
     *
     * <p>Note: Starboard assumes a single active video surface at any given time for punch-out
     * video decoding. During activity recreation or overlapping transitions, the newest activity's
     * surface supersedes any previous surface as the active target.
     */
    ...
    public void onVideoSurfaceDestroyed(Surface surface) {
        ...
        synchronized (mVideoSurfaceLock) {
            if (isActivityLifecycleCoordinationEnabled()) {
                // If this destruction is for a surface that is no longer active (e.g. from an old
                // activity
                // during an overlapping activity transition), ignore it to avoid tearing down the
                // new
                // surface.
                if (mVideoSurface != surface) {
                    Log.i(
                            TAG,
                            String.format(
                                    "onVideoSurfaceDestroyed: Ignoring destroyed surface %s from"
                                            + " stale activity; current surface is %s",
    ...
    public CobaltService openCobaltService(long nativeService, String serviceName) {
        ...
        if (service != null) {
            ...
            Log.i(
                    TAG,
                    String.format(
                            "Opened platform service %s [handle: 0x%x].",
                            serviceName, nativeService));
        }
        return service;
    }
    ...
    public CobaltService getOpenedCobaltService(String serviceName) {
        ...
        if (count > 1) {
            Log.w(
                    TAG,
                    String.format(
                            "Multiple open CobaltService instances (%d) found for %s; returning"
                                    + " first instance.",
                            count, serviceName));
        }
    ...
    public boolean getWasLowMemoryKilled() {
        ...
        ActivityManager am =
                (ActivityManager) mAppContext.getSystemService(Context.ACTIVITY_SERVICE);
        if (am == null) {
            return false;
        }
