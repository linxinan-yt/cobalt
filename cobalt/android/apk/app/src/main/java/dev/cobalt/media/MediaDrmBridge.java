...
public class MediaDrmBridge {
    ...
    private static class OperationResult {
        ...
        // Descriptive error message or details, in the scenario where the update session call
        // failed.
    ...
    void closeSession(byte[] sessionId) {
        try {
            ...
            if (!sessionExists(sessionId)) {
                Log.e(
                        TAG,
                        "Invalid sessionId in closeSession(): sessionId="
    ...
    private MediaDrmBridge(String keySystem, UUID schemeUUID, long nativeMediaDrmBridge)
        ...
        if (!isNativeMediaDrmBridgeValid()) {
            throw new IllegalArgumentException(
                    String.format(
                            "Invalid nativeMediaDrmBridge value: |%d|.", nativeMediaDrmBridge));
        }
        ...
        mMediaDrm.setOnEventListener(
                new OnEventListener() {
                    @Override
                    public void onEvent(
                            MediaDrm md, byte[] sessionId, int event, int extra, byte[] data) {
    ...
    private void handleKeyRequiredEvent(byte[] sessionId, byte[] data) {
        ...
        if (!mSessionIds.containsKey(sessionIdByteBuffer)) {
            Log.e(
                    TAG,
                    "HandleKeyRequiredEvent failed: invalid session id="
                            + bytesToString(sessionId));
            return;
        }
     ...
     *     following two cases: 1. MediaCrypto is successfully created and notified. 2. Device is
     *     not provisioned and MediaCrypto creation will be tried again after the provisioning
