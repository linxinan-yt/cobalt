...
assertEquals("TimeZone::getCanonicalID(bogus) should fail", static_cast<int32_t>(U_ILLEGAL_ARGUMENT_ERROR), ec);    assertTrue("TimeZone::getCanonicalID(bogus) should return the dest string", pResult == &canonicalID);

    // U_FAILURE on input.
    UnicodeString berlin("Europe/Berlin");
    ec = U_MEMORY_ALLOCATION_ERROR;
    pResult = &TimeZone::getCanonicalID(berlin, canonicalID, ec);
assertEquals("TimeZone::getCanonicalID(failure) should fail", static_cast<int32_t>(U_MEMORY_ALLOCATION_ERROR), ec);    assertTrue("TimeZone::getCanonicalID(failure) should return the dest string", pResult == &canonicalID);
...
assertEquals(inputID + " should fail", static_cast<int32_t>(U_ILLEGAL_ARGUMENT_ERROR), sts);            assertTrue(inputID + " should set bogus", ianaID.isBogus());
        } else {
