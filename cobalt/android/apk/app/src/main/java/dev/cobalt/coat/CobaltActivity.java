...
public abstract class CobaltActivity extends BaseCobaltActivity {
    ...
    protected void createContent(final Bundle savedInstanceState) {
        ...
        mA11yHelper =
                new CobaltA11yHelper(
                        this, mShellManager.getContentViewRenderView().getAnchorView());
        ...
        // If the app is used to start other web applications e.g google.com, because there will
        // have no
                    ...
                    .startBrowserProcessesAsync(
                            ...
                            new BrowserStartupController.StartupCallback() {
                                ...
                                public void onSuccess(
                                    ...
                                    // NOTE: This log message is hard-coded in smoke tests to detect
                                    // browser startup
                                    // success.
                                    // See ManekiBaseDeviceUtil.CHROBALT_BROWSER_READY_REGEX in the
                                    // internal test
                                    // suite.
                                    Log.i(TAG, "Browser process init succeeded");
                                    ...
                                    if (isDestroyed() || isFinishing()) {
                                        Log.w(
                                                TAG,
                                                "Activity is finishing or destroyed; skipping"
    ...
    private void finishInitialization(Bundle savedInstanceState) {
        ...
        // browser client's feature list and field trials are initialized. The feature list and
        // field
        ...
        // onWebContentsReady()
        // to only continue with initializeJavaBridge() and setting the webContents once it's
        // confirmed
    ...
    protected boolean dispatchKeyEventToIme(int keyCode, int action) {
        ...
        // |KeyEvent| needs to be created with |downTime| and |eventTime| set. If they are not set
        // the
        // app closes.
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        return dispatchKeyEventToIme(keyCode, KeyEvent.ACTION_DOWN)
    ...
    public void reloadUrl(@Nullable String targetUrl) {
        ...
        if (currentUrl.isEmpty()) {
            if (webContents != null) {
                Log.i(
                        TAG,
                        "Visible URL and fallback URL are empty, reloading without adding retry"
                                + " param");
        ...
        return getJavaSwitches().containsKey(JavaSwitches.USE_STARBOARD_LIFECYCLE)
                || (CommandLine.isInitialized()
                        && CommandLine.getInstance()
                                .hasSwitch(JavaSwitches.USE_STARBOARD_LIFECYCLE_SWITCH));
    }
        ...
        runOnUiThread(
                new Runnable() {
                    ...
                    public void run() {
                        ...
                        // Since mVideoSurfaceView is added directly to the Activity's content view,
                        // which is a
                        // FrameLayout, we expect its layout params to become
                        ...
                        // Even though as a NativeActivity we're not using the Android UI framework,
                        // by setting
                        // the  layout params it will force a layout to be requested. That will
                        // cause the
                        // SurfaceView to position its underlying Surface to match the screen
                        // coordinates of
                        // where the view would be in a UI layout and to set the surface transform
                        // matrix to
                    ...
                    new OnBackInvokedCallback() {
                        ...
                        public void onBackInvoked() {
                            if (activity.mPhysicalBackKeyPressed) {
                                return; // Bypassed: physical key events are already driving this
                                // navigation
                            }

                            // 1. Dispatch keydown to initiate navigation immediately.
                            activity.dispatchKeyEventToIme(
                                    KeyEvent.KEYCODE_BACK, KeyEvent.ACTION_DOWN);

                            // 2. Simulate physical key release with a 100ms delay.
                            // This mimics natural user latency and prevents the web page's focus
                            // manager
                            // from receiving 'keyup' prematurely during asynchronous page
                            // transitions
                            // (which would otherwise disrupt cursor/spatial navigation focus
                            // restoration).
                                    ...
                                    new Runnable() {
                                        ...
                                        public void run() {
                                            if (activity.isDestroyed() || activity.isFinishing()) {
                                                return; // Avoid memory leaks or dispatching keyup
