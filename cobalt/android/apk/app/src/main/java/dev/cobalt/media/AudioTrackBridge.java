...
public class AudioTrackBridge {
    ...
    // Pre-allocated byte[] or float[] shared with C++ via getPreAllocatedAudioDataAs*Array() to
    // avoid
    ...
    private static Object createAudioDataArray(int sampleType, int maxSamplesPerWrite) {
        switch (sampleType) {
            ...
            default:
                // Throwing RuntimeException crashes the app, which is intended since the invariant
                // is
                // broken.
    ...
    public AudioTrackBridge(
        ...
        if (mTunnelModeEnabled) {
            // Android 9.0 (Build.VERSION.SDK_INT >= 28) support v2 sync header that aligns sync
            // header
            ...
            final boolean isSurround =
                    sampleType == AudioFormat.ENCODING_AC3
                            || sampleType == AudioFormat.ENCODING_E_AC3;
        ...
        // AudioTrack.getMinBufferSize(). Especially for tunnel mode, it would fail if audio HAL
        // does
    ...
    private void play() {
        ...
        try {
            ...
            Log.e(
                    TAG,
                    String.format(
                            Locale.US, "Unable to play audio track, error: %s", e.toString()));
        }
    }
    ...
    private void flush() {
        try {
            ...
            mAudioTrack.flush();
            // Reset the states to allow reuse of |audioTrack| after flush() is called. This can
            // reduce
    ...
    private int writeWithAvSync(
        ...
        if (!mTunnelModeEnabled) {
            throw new RuntimeException(
                    "writeWithAvSync() is called when tunnelModeEnabled is false.");
        }

        long presentationTimeInNanoseconds = presentationTimeInMicroseconds * 1000;

        // Android support tunnel mode from 5.0 (API level 21), but the app has to manually write
        // the
        // sync header before API 23, where the write() function with presentation timestamp is
        // introduced.
        // Set the following constant to |false| to test manual sync header writing in API level 23
        // or
    ...
    private AudioTimestamp getAudioTimestamp() {
        ...
        // The `synchronized` is required as `maxFramePositionSoFar` can also be modified in
        // flush().
