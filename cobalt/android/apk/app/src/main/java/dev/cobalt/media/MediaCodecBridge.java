...
class MediaCodecBridge {
                ...
                new MediaCodec.Callback() {
                    ...
                    public void onError(MediaCodec codec, MediaCodec.CodecException e) {
                        synchronized (mNativeBridgeLock) {
                            ...
                            // Always report codec errors, even when we are flushing, as they
                            // indicate
                    ...
                    public void onOutputFormatChanged(MediaCodec codec, MediaFormat format) {
                        synchronized (mNativeBridgeLock) {
                            ...
                            MediaCodecBridgeJni.get()
                                    .onMediaCodecOutputFormatChanged(mNativeMediaCodecBridge);
                            if (mFrameRateEstimator != null) {
            ...
            mFrameRendererListener =
                    new MediaCodec.OnFrameRenderedListener() {
                        ...
                        public void onFrameRendered(
                                MediaCodec codec, long presentationTimeUs, long nanoTime) {
                            synchronized (mNativeBridgeLock) {
                                if (mNativeMediaCodecBridge == 0) {
                                    return;
                                }
    ...
    public static void createVideoMediaCodecBridge(
            ...
            String mime,
            String decoderName,
            // `widthHint` and `heightHint` are used to create the Android video format, which don't
            // have
        ...
        try {
            ...
            String message =
                    String.format(
                            Locale.US,
                            "Failed to create MediaCodec: %s, mustSupportSecure: %s,"
                                    + " DecoderName: %s",
                            mime,
                            crypto != null,
                            decoderName);
        ...
        boolean shouldConfigureHdr =
                colorInfo != null
                        && MediaCodecUtil.isHdrCapableVideoDecoder(mime, codecCapabilities);
        ...
        if (maxWidth > 0 && maxHeight > 0) {
            ...
            Log.i(
                    TAG,
                    "maxWidth and maxHeight not passed in, using result of"
                            + " getSupportedWidths()/Heights() (%d, %d)",
                    maxWidth,
                    maxHeight);
        }
        ...
        if (fps > 0) {
            if (videoCapabilities.areSizeAndRateSupported(maxWidth, maxHeight, fps)) {
                ...
                Log.w(
                        TAG,
                        "maxWidth and maxHeight (%d, %d)@%d not supported per"
                                + " `areSizeAndRateSupported()`, continue searching",
                        maxWidth,
                        maxHeight,
                        fps);
                if (maxHeight >= 4320
                        && videoCapabilities.areSizeAndRateSupported(7680, 4320, fps)) {
                    maxWidth = 7680;
                    maxHeight = 4320;
            ...
            if (maxHeight >= 480 && videoCapabilities.isSizeSupported(maxWidth, maxHeight)) {
                ...
                // resolution with
                // height more than 480p to minimize production impact.  To use a lower resolution
                // is more
                // to reduce memory footprint, and optimize for lower resolution isn't as helpful
                // anyway.
                Log.i(
                        TAG,
        ...
        if (!bridge.configureVideo(
            ...
            bridge.release();
            // outCreateMediaCodecBridgeResult.mErrorMessage is set inside configureVideo() on
            // error.
            return;
        }
    ...
    public void release() {
        try {
            ...
            if (android.os.Build.VERSION.SDK_INT == android.os.Build.VERSION_CODES.R) {
                Log.i(
                        TAG,
                        "Skipping stop() during destruction to avoid Android 11 framework bug:"
                                + " codec="
                                + mCodecName);
            } else {
                try {
            ...
            Log.e(
                    TAG,
                    "Exception or Error during MediaCodecBridge release(): codec=" + mCodecName,
                    t);
        }
    }
    ...
    private int queueInputBuffer(
        ...
        // Avoid update frame rate for inputs when tunnel mode is not enabled for better
        // performance.
    ...
    private boolean configureVideo(
            ...
            CreateMediaCodecBridgeResult outCreateMediaCodecBridgeResult) {
        try {
            // Since we haven't passed the properties of the stream we're playing down to this
            // level, from
            // our perspective, we could potentially adapt up to 8k at any point. We thus request 8k
            // buffers up front, unless the decoder claims to not be able to do 8k, in which case
            // we're
            // ok, since we would've rejected a 8k stream when canPlayType was called, and then use
            // those
            ...
            outCreateMediaCodecBridgeResult.mErrorMessage =
                    "Cannot configure the video codec with IllegalArgumentException: "
                            + e.toString();
        } catch (IllegalStateException e) {
    ...
    private void setupTunnelingPlayback() {
        ...
        if (Build.VERSION.SDK_INT >= 31) {
            ...
            // documentation, but some devices don't adhere to the documentation and we need to set
            // the
            ...
            Log.w(
                    TAG,
                    "MediaCodec PARAMETER_KEY_TUNNEL_PEEK is not supported in SDK version < 31: SDK"
                            + " version="
            ...
            if (mEnableIgnoreCallbacksDuringFlushing) {
                mMediaCodec
                        ...
                        .setOnFirstTunnelFrameReadyListener(
                                mMainHandler, mFirstTunnelFrameReadyListener);
            } else {
                mMediaCodec
                        .get()
                        .setOnFirstTunnelFrameReadyListener(null, mFirstTunnelFrameReadyListener);
            }
        } else {
            Log.w(
                    TAG,
                    "MediaCodec OnFirstTunnelFrameReadyListener is not supported in SDK version <"
                            + " 31: SDK version="
                            + Build.VERSION.SDK_INT);
        }
    }
