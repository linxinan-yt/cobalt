...
timing_->UpdateDecodeTimeEstimate(decode_time, now);  decodedImage.set_processing_time(
...
decodedImage.set_timestamp_us(
  ...
  decodedImage.set_content_type(frame_info->content_type);  receive_callback_->OnFrameToRender({.video_frame = decodedImage,
                                      .qp = qp,
...
frame_info.frame_type = frame.FrameType();  callback_->Map(std::move(frame_info));
